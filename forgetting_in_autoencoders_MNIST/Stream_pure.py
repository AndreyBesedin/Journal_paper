...
feature_size = 28*28
code_size = 32
max_interval_duration = 5000 # Maximum number of images in each stream interval, then change the environment
codes_storage_size = 2 # Nb of batches of codes that we store per class
fake_batches = 0
real_batches = 0
real_buffer_size = 2 # (* batch size = number of real data samples we store for further training)
...
for interval in range(stream_duration):
  ...
  for idx_stream, (X_stream, Y_stream, _) in enumerate(stream_loader):
    ...
    inputs = X_stream.cuda()
    labels = Y_stream.cuda().long()
