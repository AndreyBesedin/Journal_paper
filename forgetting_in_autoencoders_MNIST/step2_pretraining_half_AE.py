...
classifier_dict = torch.load('./pretrained_models/classifier_5_MNIST.pth')
classifier.load_state_dict(classifier_dict)
classifier.cuda()

gen_model = AE_models.AE_MNIST(code_size)
gen_model.cuda()
...
for epoch in range(training_epochs):  # loop over the dataset multiple times
  ...
  if acc > max_accuracy:
    max_accuracy = acc
    torch.save(gen_model.state_dict(), './pretrained_models/AE_5_classes_32_code_size_MNIST.pth')
