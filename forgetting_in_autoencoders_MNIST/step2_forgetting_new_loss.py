...
for epoch in range(training_epochs):  # loop over the dataset multiple times
  ...
  for idx, (train_X, train_Y, times_reconstructed) in enumerate(train_loader):
    inputs = train_X.cuda()
    # ===================forward=====================
    ...
    classification_reconstructed = classifier(reconstructions)
    #loss_gen_rec = 0
    loss_gen_rec = opts['betta2']*MSEloss_weighted(reconstructions, inputs, times_reconstructed)
    loss_gen_cl = opts['betta1']*MSEloss_weighted(classification_reconstructed, orig_classes, times_reconstructed)
...
#    torch.save(gen_model.state_dict(), './pretrained_models/AE_32_synthetic_data_forgetting.pth')
  torch.save(accuracies, './results/forgetting_AE_LSUN_{}_real_batches_new_loss.pth'.format(real_batches_to_add)) 
