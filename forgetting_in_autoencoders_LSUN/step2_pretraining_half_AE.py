...
classifier_dict = torch.load('./pretrained_models/classifier_15_LSUN.pth')
...
for epoch in range(training_epochs):  # loop over the dataset multiple times
  ...
  if acc > max_accuracy:
    max_accuracy = acc
    torch.save(gen_model.state_dict(), './pretrained_models/AE_15_classes_32_code_size_LSUN.pth')
